/*
 * sdk/include/fs.h — User-level filesystem API
 *
 * High-level wrappers around the syscall ABI.  All functions take a
 * filespec ("A0:FOO.TXT", "FOO.TXT", "2:" ...) or operate on the current
 * filesystem context set by fs_setctx().
 */
#ifndef SDK_FS_H
#define SDK_FS_H
 ...
 * Context
 *
 * Set the filesystem context (volume + user area) for subsequent
 * operations.  Returns EOK or negative errno.
 */
 ...
 * File I/O
 *
 * Open file: "r" -> read, "w" -> write+create+trunc, "a" -> append+create.
...
/*
 * Read one line (up to '\n' or EOF) into buf; returns total bytes
 * consumed from fd.
 */
int readline(int fd, char *buf, int sz);

/*
 * Seek: SEEK_SET -> absolute, SEEK_END -> offset from end.
 * Returns new position or negative errno.
 */
...
/*
 * Set file attributes (read-only, system, archive); returns 0 or
 * negative errno.
 */
...
/* Mount volume (allocates default blocks and formats); returns 0 or
 * negative errno. */
...
/* Unmount volume (full teardown, frees all blocks); returns 0 or
 * negative errno. */
