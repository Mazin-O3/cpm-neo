...
1. The bootloader sets up its stack and calls `bios_init()`.  If `bios_init()` fails, the
   bootloader halts silently.
...
(RAM-loaded) disk, and any `CONFIG_XIP_BASE` value is ignored.

### Who runs in place

Only the **kernel** and the **CCP** execute from flash. User `.com` files are
not position-independent, so they are always RAM-loaded.
...
CP/M Neo is single-task, so an in-place program can never be corrupted by a
concurrent process.
